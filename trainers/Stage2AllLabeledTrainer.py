...
class Stage2AllLabeledTrainer(StageTrainer):
	...
	def __init__(self, config):
		...
		if self.unknown_mode == 'dis':
			self.G_fixed = models.Generator(*model_config, self.enc_gen_fc_layers, self.labeled_size)
	...
	def iter_func(self):
		...
		if self.has_rec_branch:
			...
			rec.backward(rec_grad + rec_dis_grad + rec_cla2_grad)
		...
		cross.backward(cross_dis_grad + cross_cla2_grad)
	...
	def finalize(self):
		...
		if self.use_embedding:
			...
			lcode_train = run_encoder(encode_loader, self.S, self.device)[0]
			...
			encode_loader = torch.utils.data.DataLoader(encode_dataset, batch_size = self.batch_size, num_workers = self.num_workers)
			lcode_test = run_encoder(encode_loader, self.S, self.device)[0]
